...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_osgi_bundle_manifest", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_osgi_bundle_manifest": ["*.pyi", "py.typed"],
        "tree_sitter_osgi_bundle_manifest.queries": ["*.scm"],
    },
    ext_package="tree_sitter_osgi_bundle_manifest",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_osgi_bundle_manifest/binding.c",
                "src/parser.c",
            ],
            define_macros=[
